class Globalization {
  ...
  static const String back = "back"; // 输出功率
  static const String cureTime = "cure time"; // 输出功率
  static const String openTime = "shake run time"; // 输出功率
  static const String setTime = "setting time"; // 输出功率
  static const String startTimeDate = "startTime"; // 输出功率
  static const String endTimeDate = "endTime"; // 输出功率
  static const String man = "man"; // 输出功率
  static const String nv = "girl"; // 输出功率
  static const String hint_001 = "Error in obtaining user information. Please log out and log in again"; // 输出功率
  static const String hint_002 = "Please enter the treatment method"; // 输出功率
  static const String hint_003 = "The file is exported successfully"; // 输出功率
  static const String hint_004 = "Failed to obtain permissions. Please open internal storage permissions"; // 输出功率
  static const String hint_005 = "Please select a time period"; // 输出功率
  static const String hint_006 = "Data not found"; // 输出功率
  ...
  static const String ExportAll= "All"; // 输出功率
  static const String therapyMethod= "therapy method"; // 输出功率
  static const String derivedRecord= "derived record"; // 输出功率
  static const String date= "date"; // 输出功率
  static const String AttentionFour =
      "1. After the electrode pads are placed on the human body, do not operate the power switch ofthe treatment system, otherwise an instantaneous electric shock may occur. Before the treatment, the power switch must be turned on, and then the electrode pads must be placed on thehuman body, Remove the pads from the patient before turning off the power switch.\n"
      "2. During treatment, the intensity should follow the principle of increasing from small to large.and the intensity should be subject to the patient's tolerance. lf there is a power failure, pleaseturn off the power switch in time and remove the electrode pads from the patient.\n"
      "3. Self-adhesive electrode pads can only be used by themselves and cannot be shared withothers to avoid cross-infection; other electrode pads in contact with users should be washed withwater before and after treatment, and then use a medical cotton ball dipped in 75% medicalalcohol, use it after wiping and disinfecting the conductive surface of the electrode sheet.\n"
      "4. The electrode pads should be in close and even contact with the skin during use.\n"
      "5. When using the suction electrode for interferential electrotherapy for the first time, youshould click the  icon on the operation interface to perform the drainage operation, and waitfor 15s to stop the drainage before the treatment can be performed.\n"
      "6. The suction pressure should be moderate. Long-term negative pressure may cause red ancpurple skin or even blisters at the suction electrode. Therefore, after the suction electrode isplaced, the pressure should be adjusted to ensure that the suction electrode does not fall offMinimum value to avoid the above situation.\n"
      "7. The use of electrodes near the chest increases the risk of fibrillation. Avoid stimulation nearthe chest during treatment.\n";
  static const String AttentionThree =
      "1. The use of flammable anesthetics or oxidizing gases such as nitrous oxide (N,O) and oxygen(0,) must be avoided; certain materials such as cotton wool, when enriched with oxygen, wilbe ignited by the high temperature generated by the equipment; used for cleaning , sterilizedsolvents and flammable solutions should be volatilized before using the infrared polarized lightmodule; avoid the danger ofignition of the internal gas.\n"
      "2. During use, it should be noted that the radiation distance should be greater than lcm; the eyesor genitals should be prevented from being exposed to direct or scattered radiation, and it isstrictly forbidden to look directly at the radiation window.\n"
      "3. During the treatment, if the patient has any discomfort or unexpected situation, please pressthe emergency stop switch, and the treatment should be stopped immediately.\n";
  ...
  static const String detailsSeven =
      "Medium frequency/interferential module includes two treatment modes of Medium frequencyand interferential.Protocol 1-50 are medium frequency treatment protocol. In the medium frequency treatmentmode. there are two treatment channels, channel 1 and channel 2. which do not interfere witheach other;Protocol 51-60 are interferential electrotherapy protocol. In the interferential electrotherapymode.two channels work simultaneously.Medium frequency operation instructions:\n"
      ...
      "4. Click on the icon  , there will be a voice prompt of \"Start Medium Frequency Therapychannel 1\", the intensity can be adjusted (adjustable from 0 to 99, the step is 1). Please adjustthe intensity slowly. During the adjustment process, keep asking the patient how they feel .\n"
      "5. When the set treatment time is over or the output is stopped manually, there will be a voiceprompt of\"Stop Medium Frequency Therapy channel 1\"\n"
      ...
      "1. If you choose negative pressure interferential electrotherapy output, you need to use foursuction cups. If it is interferential electrotherapy output, you need to use four interferentiaelectrodes.\n"
      "2.The protocol selection range is 51~60.\n"
      "3. The treatment time is already within the protocol and does not need to be set.\n"
      "4. Click on the icon  , there will be a voice prompt of\"Start Interferential Electrotherapy\", theintensity can be adjusted (adjustable from 0 to 99, the step is 1). Please adjust the outputintensity slowly. If it is negative pressure suction therapy, adjust the suction pressure. Duringthe adjustment process, keep asking the patient how they feel.\n"
      "5. When the set treatment time ends or the output is stopped manually, there will be a voiceprompt of\"Stop Interferential Electrotherapy\"\n"
      "6. The icons of\"+ -\"are gray to indicate that they are not selectable.\n";
